...
public class TestBasePage {
    ...
    public static void explicitWaitByWebElemt(WebElement elementToVisible) {
        explicitWait.until(ExpectedConditions.visibilityOf(elementToVisible));
    }
    ...
    public static void selectBubbleRating(String xpathForBubbleRatingElement,int startCountToSelect) {
        actions.moveToElement(new WebDriverWait(driver, 20)
                .until(ExpectedConditions.visibilityOfElementLocated
                        (By.xpath(xpathForBubbleRatingElement))), startCountToSelect*10, 0)
                .click()
                .build()
                .perform();
    }
    public static String getHotelServiceBubbleRatingXpath(String serviceType){
        return ("//div[text()='"+ serviceType + "']//parent::div//span");
    }
  public static void seleniumAlert(String message){
        String argument = "alert('"+message+"')";
        javascriptExecutor.executeScript(argument);
  }
}
